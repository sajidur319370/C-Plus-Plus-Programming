// ******** Snake Video Game ************
// ****** The Game is only compiled by GCC compiler***********
// Only for MS Windows
...
private:
    ...
    enum direction
    {
        ...
        LEFT,
        RIGHT
    }; // directions of motion(w.r.t player in front of screen)
    direction dir;                        // current direction of motion of snake's head
    unordered_set<position, MyHash> uset; // set stores unique body parts
public:
    // Constructor
    Snake(int x = 3, int y = 3) : body_head_symbol('@'), body_part_symbol('+'), dir(DOWN), body_size(1)
    {
